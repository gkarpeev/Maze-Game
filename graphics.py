...
class ConsoleGraphics:
    ...
    def work(self, solver):
        ...
        self.finish_cell = Cell(self.maze.get_height() - 1, self.maze.get_width() - 1)

        self.start_cell.x, self.start_cell.y = map(int, input('Please, write Start point\n').split())
        self.finish_cell.x, self.finish_cell.y = map(int, input('Please, write End point\n').split())

        path, visited_cells = solver.solve(self.maze, self.start_cell, self.finish_cell)
...
class GameGraphics:
    ...
    def __init__(self, maze: Maze):
        self.maze = maze
        self.tile = min(30, (1200 - 2 * self.WIDTH_INDENT) // maze.width, (900 - 2 * self.HEIGHT_INDENT) // maze.height)
        self.width = min(1200, maze.width * self.tile + 2 * self.WIDTH_INDENT) + 3
        self.height = min(900, maze.height * self.tile + 2 * self.HEIGHT_INDENT) + 3
    ...
    def work(self, solver):
        ...
        while True:
            ...
            if self.start_cell != Cell(-1, -1) and self.finish_cell != Cell(-1, -1):
                path, vis = solver.solve(self.maze, self.start_cell, self.finish_cell)
    ...
    def draw_cell(self, cell: Cell, color):
        x, y = cell.x * self.tile + self.HEIGHT_INDENT, cell.y * self.tile + self.WIDTH_INDENT
    ...
    def draw_wall(self, first_cell: Cell, second_cell: Cell):
        ...
        if Maze.is_horizontal_line(first_cell, second_cell):
            ...
            x, y = second_cell.x * self.tile + self.HEIGHT_INDENT, second_cell.y * self.tile + self.WIDTH_INDENT
            pygame.draw.line(self.window, Color.Wall.value, (y, x), (y + self.tile, x), 3)
        else:
            if first_cell.y > second_cell.y:
                first_cell, second_cell = second_cell, first_cell
            x, y = second_cell.x * self.tile + self.HEIGHT_INDENT, second_cell.y * self.tile + self.WIDTH_INDENT
            pygame.draw.line(self.window, Color.Wall.value, (y, x), (y, x + self.tile), 3)
